...
import { approveWithdrawal, executeWithdrawal, getPendingWithdrawals } from "@/src/services/wallet-api";
...
export function MultisigPanel() {
  ...
  const approve = useApiMutation((input: { id: string; address: string }) => approveWithdrawal(input.id, input.address));
    ...
    <section className="page">
      ...
      <PermissionGuard>
        <Card title="待审批提现">
                    ...
                    <tr key={item.withdrawal_id}>
                      ...
                      <td>
                        <div className="button-row">
                          <Button variant="secondary" icon={<CheckCircle2 size={16} />} disabled={!wallet.address || approve.loading} onClick={() => wallet.address && approve.mutate({ id: item.withdrawal_id, address: wallet.address })}>审批</Button>
                          <Button icon={<PlayCircle size={16} />} disabled={!wallet.address || execute.loading} onClick={() => wallet.address && execute.mutate({ id: item.withdrawal_id, address: wallet.address })}>执行</Button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          {approve.data ? <div className="state">审批数：{approve.data.approved_count}/{approve.data.threshold}</div> : null}
